...
__global__ static void filter2d_8_kernel(uchar const* yin, uchar *yout, int n_rows, int n_cols, int K) {
...
//     float *device_convol;
    ...
    filter2d_8_kernel<<<dimGrid, dimBlock>>>(device_in, device_out, rows, cols, K);
