...
//! Performs benchmarking of median function
static void median(benchmark::State& state, int width, int height, int ksize)
{
    ...
    for(auto _ : state)
    {
        median(src, dst, ksize);
    }
}

BENCHMARK_CAPTURE(median, 640x480x9, 640, 480, 9)
    ->UseRealTime();
